...
* For Docker Desktop 4.34 and 4.35 only: The `allowDerivedImages` setting
...
We recommend using this only in scenarios where explicitly listing allowed
container images is not flexible enough.
...
* It's not possible to allow Docker socket bind-mounts on containers using
  ...
  That's because Docker Desktop pulls the digests for the allowed images from
